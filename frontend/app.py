...
if "file_uploader_key" not in st.session_state:
    st.session_state.file_uploader_key = str(uuid.uuid4()) # Initialize with a unique key
...
section.main .block-container {
    ...
    background-color: #000000; /* Ensuring content visibility */
    padding: 2rem 2.5rem;
    border-radius: 10px;
...
def vision_ai_page():
    ...
    with action_cols[1]:
        ...
        if st.button("🗑️ Clear All", use_container_width=True, disabled=clear_disabled):
            ...
            st.session_state.uploaded_image_info = None
            st.session_state.caption_data = None
            st.session_state.file_uploader_key = str(uuid.uuid4()) # Generate a new unique key
